...
import { INV_CC, pickQuery, pickList, pickForm } from '@/config/fields';
...
type LineRow = SalesAllowanceLine & { _key: string; spc?: string };
...
const head = reactive<SalesAllowanceHead>({
  ps_no: '',
  ps_dd: today(),
  cus_no: '',
  tax_id: '2',
...
const list = ref<SalesAllowanceHead[]>([]);
...
/** 字段配置 — 同步 analysis/字段对照/销货折让.md */
const headQueryFields = pickQuery(INV_CC.head);
const headListFields = pickList(INV_CC.head);
const lineFormFields = pickForm(INV_CC.line);
...
const openShipments = ref<OpenSalesShipment[]>([]);
...
async function loadList() {
  loading.value = true;
  try {
    const { data } = await api.salesAllowanceList({ limit: 200 });
    list.value = data;
  } finally {
    loading.value = false;
  }
...
async function onNew() {
  const { data } = await api.nextSalesAllowanceNo();
  Object.assign(head, {
    ps_no: data.ps_no,
    ps_dd: today(),
    cus_no: '',
...
async function openSaTransfer() {
  if (!head.cus_no) {
    ElMessage.warning('请先选择销货客户');
    return;
  }
  const { data } = await api.openSalesShipments(head.cus_no);
  if (!data.length) {
    ElMessage.info('该客户没有可折让的销货单');
    return;
  }
  openShipments.value = data;
  saPicker.value = true;
}

async function transferFromSa(psNo: string) {
  try {
    const { data } = await api.salesShipmentAllowanceLines(psNo);
    const h = data.head;
    head.os_id = 'SA';
    head.os_no = h.ps_no || '';
    ...
    saPicker.value = false;
    ElMessage.success(`已从销货单 ${psNo} 带入表身`);
  } catch {
...
async function onSave() {
  ...
  try {
    if (editing.value) {
      await api.updateSalesAllowance(editing.value, payload);
      ElMessage.success('更新成功');
    } else {
      await api.createSalesAllowance(payload);
...
async function openEdit(row: SalesAllowanceHead) {
  await openAllowance(row.ps_no);
}

async function openAllowance(psNo: string) {
  const { data } = await api.getSalesAllowance(psNo);
...
const {
  ...
  editing,
  list,
  openBill: openAllowance,
  resetNew: onNew,
  reloadList: loadList,
  deleteBill: (no) => api.deleteSalesAllowance(no).then(() => undefined),
});
...
onMounted(async () => {
  try {
    ...
    ElMessage.error('加载销货折让失败');
  }
});
</script>

<template>
  <ErpBillPage title="销货折让" subtitle="顶部查询 · 表头表身页内维护 · 下方浏览历史单据" :loading="loading" :menu-code="INV_CC.menuCode">
    <template #toolbar>
    ...
    <template #header>
      ...
      <el-form label-width="96px" label-position="right" class="erp-bill-header-form">
          ...
          <el-row :gutter="12">
            <el-col :span="12">
              <el-form-item label="销货客户" required>
          ...
          <ErpBillHeadExtFields v-model="head.ext_fields" :menu-code="INV_CC.menuCode" />
        </el-form>
    </template>
  ...
  <LookupDialog
    ...
    :data="openShipments"
    @select="(r: OpenSalesShipment) => transferFromSa(r.ps_no)"
  />
