...
import { INV_CA, pickQuery, pickList, pickForm } from '@/config/fields';
...
type LineRow = SalesShipmentLine & { _key: string; spc?: string };
...
const head = reactive<SalesShipmentHead>({
  ps_no: '',
  ps_dd: today(),
  cus_no: '',
  tax_id: '2',
...
const list = ref<SalesShipmentHead[]>([]);
...
/** 字段配置 — 同步 analysis/字段对照/销货单.md */
const headQueryFields = pickQuery(INV_CA.head);
const headListFields = pickList(INV_CA.head);
const lineFormFields = pickForm(INV_CA.line);
...
const openOrders = ref<OpenSalesOrder[]>([]);
...
async function loadList() {
  loading.value = true;
  try {
    const { data } = await api.salesShipmentList({ limit: 200 });
    list.value = data;
  } finally {
    loading.value = false;
  }
...
async function onNew() {
  const { data } = await api.nextSalesShipmentNo();
  Object.assign(head, {
    ps_no: data.ps_no,
    ps_dd: today(),
    cus_no: '',
...
async function openSoTransfer() {
  if (!head.cus_no) {
    ElMessage.warning('请先选择销货客户');
    return;
  }
  const { data } = await api.openSalesOrders(head.cus_no);
  if (!data.length) {
    ElMessage.info('该客户没有未交完的销售订单');
    return;
  }
...
async function transferFromSo(osNo: string) {
  try {
    const { data } = await api.salesOrderShipLines(osNo);
    const h = data.head;
    head.os_id = 'SO';
    head.os_no = h.os_no;
    ...
    ElMessage.success(`已从销售订单 ${osNo} 带入表身`);
  } catch {
...
async function onSave() {
  ...
  try {
    if (editing.value) {
      await api.updateSalesShipment(editing.value, payload);
      ElMessage.success('更新成功');
    } else {
      await api.createSalesShipment(payload);
...
async function openShipment(psNo: string) {
  const { data } = await api.getSalesShipment(psNo);
...
async function openEdit(row: SalesShipmentHead) {
...
const {
  ...
  resetNew: onNew,
  reloadList: loadList,
  deleteBill: (no) => api.deleteSalesShipment(no).then(() => undefined),
});
...
onMounted(async () => {
  try {
    ...
    ElMessage.error('加载销货单失败');
  }
});
</script>

<template>
  <ErpBillPage title="销货单" subtitle="顶部查询 · 表头表身页内维护 · 下方浏览历史单据" :loading="loading" :menu-code="INV_CA.menuCode">
    <template #toolbar>
    ...
    <template #header>
      <div class="erp-bill-section-head">
        ...
        <div class="erp-bill-section-actions">
          ...
          <el-button size="small" @click="openSoTransfer">从销售订单转入</el-button>
        </div>
      </div>
      <el-form label-width="96px" label-position="right" class="erp-bill-header-form">
          <el-row :gutter="12">
            ...
            <el-col :span="6">
              <el-form-item label="转入单号" data-field="OS_NO">
                <el-input v-model="head.os_no" readonly placeholder="从销售订单转入" />
              </el-form-item>
            </el-col>
            <el-col :span="6">
          ...
          <ErpBillHeadExtFields v-model="head.ext_fields" :menu-code="INV_CA.menuCode" />
        </el-form>
    </template>
  ...
  <LookupDialog
    ...
    :data="openOrders"
    @select="(r: OpenSalesOrder) => transferFromSo(r.os_no)"
  />
