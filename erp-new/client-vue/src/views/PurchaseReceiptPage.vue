...
import { INV_BA, pickQuery, pickList, pickForm } from '@/config/fields';
...
type LineRow = PurchaseReceiptLine & { _key: string; spc?: string };
...
const head = reactive<PurchaseReceiptHead>({
  ps_no: '',
  ps_dd: today(),
  cus_no: '',
  tax_id: '2',
...
const list = ref<PurchaseReceiptHead[]>([]);
...
/** 字段配置 — 同步 analysis/字段对照/进货单.md */
const headQueryFields = pickQuery(INV_BA.head);
const headListFields = pickList(INV_BA.head);
const lineFormFields = pickForm(INV_BA.line);
...
const openOrders = ref<OpenPurchaseOrder[]>([]);
...
async function loadList() {
  loading.value = true;
  try {
    const { data } = await api.purchaseReceiptList({ limit: 200 });
    list.value = data;
  } finally {
    loading.value = false;
  }
...
async function onNew() {
  const { data } = await api.nextPurchaseReceiptNo();
  Object.assign(head, {
    ps_no: data.ps_no,
    ps_dd: today(),
    cus_no: '',
...
async function openPoTransfer() {
  if (!head.cus_no) {
    ElMessage.warning('请先选择厂商');
    return;
  }
  const { data } = await api.openPurchaseOrders(head.cus_no);
  if (!data.length) {
    ElMessage.info('该厂商没有未交完的采购单');
    return;
  }
...
async function transferFromPo(osNo: string) {
  try {
    const { data } = await api.purchaseOrderReceiptLines(osNo);
    const h = data.head;
    head.os_id = 'PO';
    head.os_no = h.os_no;
    ...
    poPicker.value = false;
    ElMessage.success(`已从采购单 ${osNo} 带入表身`);
  } catch {
...
async function onSave() {
  ...
  try {
    if (editing.value) {
      await api.updatePurchaseReceipt(editing.value, payload);
      ElMessage.success('更新成功');
    } else {
      await api.createPurchaseReceipt(payload);
...
async function openShipment(psNo: string) {
  const { data } = await api.getPurchaseReceipt(psNo);
...
async function openEdit(row: PurchaseReceiptHead) {
  await openShipment(row.ps_no);
}
...
const {
  ...
  editing,
  list,
  openBill: openShipment,
  resetNew: onNew,
  reloadList: loadList,
  deleteBill: (no) => api.deletePurchaseReceipt(no).then(() => undefined),
});
...
onMounted(async () => {
  try {
    ...
    ElMessage.error('加载进货单失败');
  }
});
</script>

<template>
  <ErpBillPage title="进货单" subtitle="顶部查询 · 表头表身页内维护 · 下方浏览历史单据" :loading="loading" :menu-code="INV_BA.menuCode">
    <template #toolbar>
      <ErpBillToolbar @search="loadList" @reset="resetQuery">
        ...
        <template #query>
          ...
          >
            ...
            <el-input
              v-else-if="f.key === 'ps_no'"
              v-model="filters.ps_no"
              clearable
              placeholder="模糊查询"
              @keyup.enter="loadList"
            />
            <el-input
              v-else-if="f.key === 'cus_no'"
              v-model="filters.cus_no"
              clearable
              placeholder="代号/名称"
              @keyup.enter="loadList"
            />
    ...
    <template #header>
      <div class="erp-bill-section-head">
        ...
        <div class="erp-bill-section-actions">
          ...
          <el-button size="small" @click="openPoTransfer">从采购单转入</el-button>
        </div>
      </div>
      <el-form label-width="96px" label-position="right" class="erp-bill-header-form">
          <el-row :gutter="12">
            <el-col :span="6">
              <el-form-item label="日期" required data-field="PS_DD">
                <el-date-picker v-model="head.ps_dd" type="date" value-format="YYYY-MM-DD" style="width: 100%" />
              </el-form-item>
            </el-col>
            <el-col :span="6">
              <el-form-item label="单号" data-field="PS_NO">
                <el-input v-model="head.ps_no" readonly />
              </el-form-item>
            </el-col>
            <el-col :span="6">
              <el-form-item label="转入单号" data-field="OS_NO">
                <el-input v-model="head.os_no" readonly placeholder="从采购单转入" />
              </el-form-item>
            </el-col>
            <el-col :span="6">
          ...
          <el-row :gutter="12">
            <el-col :span="6">
              <el-form-item label="客户代号" required data-field="CUS_NO">
                <LookupField :model-value="head.cus_no" :display="custName" @open="custPicker = true" />
              </el-form-item>
            </el-col>
            <el-col :span="6">
              <el-form-item label="业务员代号" data-field="SAL_NO">
            ...
            <el-col :span="6">
              <el-form-item label="部门代号" data-field="DEP">
          ...
          <el-row :gutter="12">
            ...
            <el-col :span="6">
              <el-form-item label="折扣" data-field="DIS_CNT">
                <el-input-number v-model="head.dis_cnt" :min="0" :max="100" style="width: 100%" />
              </el-form-item>
            </el-col>
            <el-col :span="6">
              <el-form-item label="立帐方式" data-field="ZHANG_ID">
            ...
            <el-col :span="6">
              <el-form-item label="交货方式" data-field="SEND_MTH">
                <el-input v-model="head.send_mth" />
              </el-form-item>
            </el-col>
            <el-col :span="6">
              <el-form-item label="交货仓" data-field="SEND_WH">
                <LookupField :model-value="head.send_wh" :display="sendWhName" @open="sendWhPicker = true" />
              </el-form-item>
            </el-col>
            <el-col :span="6">
              <el-form-item label="交易方式" data-field="PAY_MTH">
          ...
          <el-row :gutter="12">
            <el-col :span="6">
              <el-form-item label="发票号码" data-field="INV_NO">
                <el-input v-model="head.inv_no" readonly />
              </el-form-item>
            </el-col>
            <el-col :span="6">
              <el-form-item label="收付款单号" data-field="RP_NO">
                <el-input v-model="head.rp_no" readonly />
              </el-form-item>
            </el-col>
            <el-col :span="6">
              <el-form-item label="凭证号码" data-field="VOH_NO">
                <el-input v-model="head.voh_no" readonly />
              </el-form-item>
            </el-col>
          </el-row>
          <el-form-item label="送货地址" data-field="ADR">
          ...
          <ErpBillHeadExtFields v-model="head.ext_fields" :menu-code="INV_BA.menuCode" />
        </el-form>
    </template>
  ...
  <LookupDialog
    ...
    v-bind="getInlineLookupDialogProps({
      title: '选择采购单（未交完）',
      rowKey: 'os_no',
      searchKeys: ['os_no', 'cus_os_no'],
      columns: [
        { prop: 'os_no', label: 'OS_NO', width: 120 },
        { prop: 'os_dd', label: 'OS_DD', width: 100 },
        { prop: 'cus_os_no', label: 'CUS_OS_NO', minWidth: 120 },
      ],
    })"
    :data="openOrders"
    @select="(r: OpenPurchaseOrder) => transferFromPo(r.os_no)"
  />
