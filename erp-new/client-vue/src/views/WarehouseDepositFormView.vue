...
import type { Dept, ManufacturingOrderHead, Product, Salm, WarehouseDepositHead } from '@/api/types';
...
const props = defineProps<{
  ...
  usrName: string;
  totals: { qty: number };
  billStatus: (row: WarehouseDepositHead) => { label: string; kind: string };
  formLabels?: {
    formTitleNew: string;
...
const moColumns = [
  ...
  { prop: 'qty_fin', label: '已缴库', width: 90 },
];
...
function onMoConfirm(rows: ManufacturingOrderHead[]) {
  if (props.isFormReadonly) return;
  const nos = rows.map((r) => r.mo_no).filter(Boolean) as string[];
  emit('transferFromMos', nos);
  moPicker.value = false;
}

function onMoSelectSingle(row: ManufacturingOrderHead) {
  if (props.isFormReadonly) return;
  if (row.mo_no) emit('transferFromMos', [row.mo_no]);
  moPicker.value = false;
}
</script>
...
<template>
  <ErpKingdeeBillEdit
    :title="formTitle"
    :bill-no="head.mm_no || undefined"
    :status-label="head.mm_no ? billStatus(head).label : '新建'"
    :status-kind="(head.mm_no ? billStatus(head).kind : 'open') as 'open' | 'audited' | 'closed' | 'analyzed'"
    :loading="loading"
    :menu-code="menuCode"
  >
    ...
    <template #toolbar>
      ...
      <el-button
        ...
        type="danger"
        plain
        @click="emit('deleteBill', head.mm_no!)"
      >
        删除
      </el-button>
    ...
    <template #basic>
      ...
      >
        <el-row :gutter="12">
          <el-col :span="6">
            <el-form-item :label="labels.orderNoLabel" required data-field="MM_NO">
              <el-input :model-value="head.mm_no" readonly />
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="缴库日期" data-field="MM_DD">
              <el-date-picker
                v-model="head.mm_dd"
                type="date"
          ...
          <el-col :span="6">
            <el-form-item label="制令单号" data-field="MO_NO">
              <LookupField
                :model-value="head.mo_no"
                :display="head.mo_no"
                placeholder="可选；多笔请用转入"
                :disabled="isFormReadonly"
                @open="moPicker = true"
              />
            </el-form-item>
          ...
          <el-col :span="6">
            <el-form-item label="生产部门" data-field="DEP">
              <LookupField
    ...
    <template #totals>
      <div class="kd-bill-edit__totals-row">
        <span>合计数量<strong>{{ fmtQty(totals.qty) }}</strong></span>
      </div>
    </template>
  ...
  <LookupDialog
    ...
    @confirm="(rows) => onMoConfirm(rows as unknown as ManufacturingOrderHead[])"
    @select="(row) => onMoSelectSingle(row as unknown as ManufacturingOrderHead)"
  />
  <LookupDialog
    v-model="depPicker"
  ...
  <LookupDialog
    v-model="prdPicker"
    v-bind="getLookupDialogProps('product', { title: '选择产品', dialogClass: 'kd-so-dialog' })"
