...
import { INV_CB, pickQuery, pickList, pickForm } from '@/config/fields';
...
type LineRow = SalesReturnLine & { _key: string; spc?: string };
...
const head = reactive<SalesReturnHead>({
  ps_no: '',
  ps_dd: today(),
  cus_no: '',
  tax_id: '2',
...
const list = ref<SalesReturnHead[]>([]);
...
/** 字段配置 — 同步 analysis/字段对照/销货退回.md */
const headQueryFields = pickQuery(INV_CB.head);
const headListFields = pickList(INV_CB.head);
const lineFormFields = pickForm(INV_CB.line);
...
async function loadList() {
  loading.value = true;
  try {
    const { data } = await api.salesReturnList({ limit: 200 });
    list.value = data;
  } finally {
    loading.value = false;
  }
...
async function onSave() {
  ...
  try {
    if (editing.value) {
      await api.updateSalesReturn(editing.value, payload);
...
async function openEdit(row: SalesReturnHead) {
...
const {
  ...
  resetNew: onNew,
  reloadList: loadList,
  deleteBill: (no) => api.deleteSalesReturn(no).then(() => undefined),
});
...
onMounted(async () => {
  try {
    ...
    ElMessage.error('加载销货退回失败');
  }
});
</script>

<template>
  <ErpBillPage title="销货退回" subtitle="顶部查询 · 表头表身页内维护 · 下方浏览历史单据" :loading="loading" :menu-code="INV_CB.menuCode">
    <template #toolbar>
    ...
    <template #header>
      ...
      <el-form label-width="96px" label-position="right" class="erp-bill-header-form">
          ...
          <ErpBillHeadExtFields v-model="head.ext_fields" :menu-code="INV_CB.menuCode" />
        </el-form>
    </template>
