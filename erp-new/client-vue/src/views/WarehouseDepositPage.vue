...
import WarehouseDepositFormView from '@/views/WarehouseDepositFormView.vue';
...
import { useWarehouseDepositBill } from '@/composables/useWarehouseDepositBill';
import { MRP_AFC } from '@/config/fields';
import { initErpUser } from '@/composables/useErpUser';
import type { DetailGridSummary } from '@/config/detailGridRegistry';
import type { WarehouseDepositHead } from '@/api/types';
...
function rowClassName({ row }: { row: Record<string, unknown> }) {
  return row.mm_no === head.mm_no ? 'erp-row-current-bill' : '';
}

function billRowStatus(row: Record<string, unknown>) {
  return billStatus(row as unknown as WarehouseDepositHead);
}
    ...
    <template v-if="viewMode === 'list'">
        ...
        <div class="kd-so-query__grid">
          <div v-for="f in headQueryFields" :key="f.key" class="kd-so-query-field">
            ...
            <div class="kd-so-query-field__control">
              <el-input
                v-model="filters[f.key === 'mo_no' ? 'mo_no' : 'mm_no']"
                clearable
      ...
      <div class="kd-so-card kd-so-table">
        <div class="kd-so-card__body kd-so-card__body--table">
          <ErpDetailGrid
            ref="billGridRef"
            menu-code="MrpAFC_BILL"
            :rows="billGridRows"
            :loading="loading"
            :action-width="168"
            :row-class-name="rowClassName"
            @summary-change="onBillSummaryChange"
            @row-dblclick="(row) => openBill(String(row.mm_no), true)"
          >
            <template #row-actions="{ row }">
              <el-button class="kd-so-link-btn" link type="primary" @click="openBill(String(row.mm_no), true)">
                查看
              </el-button>
              <el-button
                ...
                @click="openBill(String(row.mm_no), false)"
              >
                编辑
              </el-button>
              <el-button
                ...
                type="danger"
                :disabled="billRowStatus(row).kind !== 'open'"
                @click="onDeleteBill(String(row.mm_no))"
              >
                删除
              </el-button>
            </template>
      ...
      <div class="kd-so-table-footer">
        <div class="kd-so-table-footer__info">
          共 <strong>{{ billSummary.count }}</strong> 笔缴库单
        </div>
      </div>
    </template>

    <WarehouseDepositFormView
      v-else
      :head="head"
      :lines="lines"
      ...
      @transfer-from-mos="transferFromMos"
    />
  </div>
</template>
