...
import { INV_BC, pickQuery, pickList, pickForm } from '@/config/fields';
...
type LineRow = PurchaseAllowanceLine & { _key: string; spc?: string };
...
const head = reactive<PurchaseAllowanceHead>({
  ps_no: '',
  ps_dd: today(),
  cus_no: '',
  tax_id: '2',
...
const list = ref<PurchaseAllowanceHead[]>([]);
...
/** 字段配置 — 同步 analysis/字段对照/进货折让.md */
const headQueryFields = pickQuery(INV_BC.head);
const headListFields = pickList(INV_BC.head);
const lineFormFields = pickForm(INV_BC.line);
...
async function loadList() {
  loading.value = true;
  try {
    const { data } = await api.purchaseAllowanceList({ limit: 200 });
    list.value = data;
  } finally {
    loading.value = false;
  }
...
async function onNew() {
  const { data } = await api.nextPurchaseAllowanceNo();
  Object.assign(head, {
    ps_no: data.ps_no,
    ps_dd: today(),
    cus_no: '',
...
async function transferFromPc(psNo: string) {
  try {
    const { data } = await api.purchaseReceiptAllowanceLines(psNo);
    const h = data.head;
    head.os_id = 'PC';
    head.os_no = h.ps_no || '';
...
async function onSave() {
  ...
  try {
    if (editing.value) {
      await api.updatePurchaseAllowance(editing.value, payload);
      ElMessage.success('更新成功');
    } else {
      await api.createPurchaseAllowance(payload);
...
async function openEdit(row: PurchaseAllowanceHead) {
  await openAllowance(row.ps_no);
}

async function openAllowance(psNo: string) {
  const { data } = await api.getPurchaseAllowance(psNo);
...
const {
  ...
  resetNew: onNew,
  reloadList: loadList,
  deleteBill: (no) => api.deletePurchaseAllowance(no).then(() => undefined),
});
...
onMounted(async () => {
  try {
    ...
    ElMessage.error('加载进货折让失败');
  }
});
</script>

<template>
  <ErpBillPage title="进货折让" subtitle="顶部查询 · 表头表身页内维护 · 下方浏览历史单据" :loading="loading" :menu-code="INV_BC.menuCode">
    <template #toolbar>
    ...
    <template #header>
      ...
      <el-form label-width="96px" label-position="right" class="erp-bill-header-form">
          ...
          <ErpBillHeadExtFields v-model="head.ext_fields" :menu-code="INV_BC.menuCode" />
        </el-form>
    </template>
