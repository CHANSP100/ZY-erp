...
import MaterialIssueFormView from '@/views/MaterialIssueFormView.vue';
...
import { useMaterialIssueBill } from '@/composables/useMaterialIssueBill';
import { MRP_AG } from '@/config/fields';
import { initErpUser } from '@/composables/useErpUser';
import type { DetailGridSummary } from '@/config/detailGridRegistry';
import type { MaterialIssueHead } from '@/api/types';
...
function rowClassName({ row }: { row: Record<string, unknown> }) {
  return row.ml_no === head.ml_no ? 'erp-row-current-bill' : '';
}

function billRowStatus(row: Record<string, unknown>) {
  return billStatus(row as unknown as MaterialIssueHead);
}
    ...
    <template v-if="viewMode === 'list'">
        ...
        <div class="kd-so-query__grid">
          <div v-for="f in headQueryFields" :key="f.key" class="kd-so-query-field">
            ...
            <div class="kd-so-query-field__control">
              <el-input
                v-model="filters[f.key === 'mrp_no' ? 'mrp_no' : f.key === 'mo_no' ? 'mo_no' : 'ml_no']"
                clearable
      ...
      <div class="kd-so-card kd-so-table">
        <div class="kd-so-card__body kd-so-card__body--table">
          <ErpDetailGrid
            ref="billGridRef"
            menu-code="MrpAG_BILL"
            :rows="billGridRows"
            :loading="loading"
            :action-width="168"
            :row-class-name="rowClassName"
            @summary-change="onBillSummaryChange"
            @row-dblclick="(row) => openBill(String(row.ml_no), true)"
          >
            <template #row-actions="{ row }">
              <el-button class="kd-so-link-btn" link type="primary" @click="openBill(String(row.ml_no), true)">
                查看
              </el-button>
              <el-button
                ...
                @click="openBill(String(row.ml_no), false)"
              >
                编辑
              </el-button>
              <el-button
                ...
                type="danger"
                :disabled="billRowStatus(row).kind !== 'open'"
                @click="onDeleteBill(String(row.ml_no))"
              >
                删除
              </el-button>
            </template>
      ...
      <div class="kd-so-table-footer">
        <div class="kd-so-table-footer__info">
          共 <strong>{{ billSummary.count }}</strong> 笔生产领料
          · 合计领料数量 <strong>{{ fmtQty(billSummary.qty) }}</strong>
        </div>
      </div>
    </template>

    <MaterialIssueFormView
      v-else
      :head="head"
      :lines="lines"
      ...
      :dep-name="depName"
      :wh-mtl-name="whMtlName"
      :usr-name="usrName"
      :totals="totals"
      :bill-status="billStatus"
      :form-labels="formLabels"
      :menu-code="MRP_AG.menuCode"
      @go-list="goList"
      @go-add="goAdd"
      @save="onSave"
      @audit="onAudit"
      ...
      @transfer-from-mo="transferFromMo"
    />
  </div>
</template>
