...
import {
  ...
  type Cust,
  type OpenSalesOrder,
  type SalesShipmentHead,
  type SalesShipmentLine,
  type Salm,
  type Warehouse,
} from '../api';

type LineRow = SalesShipmentLine & { key: string };
...
export default function SalesShipmentPage() {
  const [form] = Form.useForm<SalesShipmentHead>();
  const [lines, setLines] = useState<LineRow[]>([]);
  const [shipments, setShipments] = useState<SalesShipmentHead[]>([]);
  ...
  const [soPicker, setSoPicker] = useState(false);
  ...
  const load = async () => {
    const [sh, c, w, s] = await Promise.all([
      api.salesShipmentList({ limit: 100 }),
  ...
  useEffect(() => {
    load().catch(() => message.error('加载销货单失败'));
  }, []);
  ...
  const onNew = async () => {
    ...
    form.setFieldsValue({
      ...
      dis_cnt: 0,
      os_id: 'SO',
    } as unknown as SalesShipmentHead);
    setLines([]);
    setEditing(null);
  };
  ...
  const openSoTransfer = async () => {
    const cus = form.getFieldValue('cus_no');
    if (!cus) return message.warning('请先选择销货客户');
    const { data } = await api.openSalesOrders(String(cus));
    if (!data.length) return message.info('该客户没有未交完的受订单');
    setOpenOrders(data);
    setSoPicker(true);
  };

  const transferFromSo = async (osNo: string) => {
    try {
      ...
      setSoPicker(false);
      message.success(`已从受订单 ${osNo} 带入表身`);
    } catch {
      message.error('转入失败');
    }
  };
  ...
  const onSave = async () => {
    ...
    try {
      if (editing) {
        await api.updateSalesShipment(editing, payload);
  ...
  const openShipment = async (psNo: string) => {
  ...
  return (
    <div style={{ padding: 16 }}>
        ...
        <Col span={16}>
          <Card
            ...
            extra={
              <Space>
                ...
                <Button onClick={openSoTransfer}>从受订单转入</Button>
          ...
          >
            <Form form={form} layout="vertical">
              <Row gutter={8}>
                <Col span={6}>
                  <Form.Item name="ps_no" label="销货单号">
                    <Input readOnly />
                  </Form.Item>
                </Col>
                <Col span={6}>
                  <Form.Item name="ps_dd" label="销货日期" rules={[{ required: true }]}>
                    <DatePicker style={{ width: '100%' }} />
                  </Form.Item>
                </Col>
                <Col span={6}>
                  <Form.Item name="os_no" label="转入单号（受订单）">
                    <Input readOnly placeholder="从受订单转入" />
                  </Form.Item>
                </Col>
                <Col span={6}>
              ...
              <Row gutter={8}>
                <Col span={12}>
                  <Form.Item name="cus_no" label="销货客户" rules={[{ required: true }]}>
            ...
            <Table
              ...
              columns={[
                ...
                { title: '对方货号', dataIndex: 'sup_prd_no', width: 90, ellipsis: true },
                { title: '来源单号', dataIndex: 'os_no', width: 100 },
              ]}
            />
        ...
        <Col span={8}>
          <Card title="销货单列表">
            <Table
              ...
              onRow={(r) => ({ onDoubleClick: () => openShipment(r.ps_no) })}
              columns={[
      ...
      <LookupModal
        ...
        onSelect={(r) => transferFromSo(r.os_no)}
        columns={[
          { title: '受订单号', dataIndex: 'os_no', width: 120 },
          { title: '日期', dataIndex: 'os_dd', width: 100 },
