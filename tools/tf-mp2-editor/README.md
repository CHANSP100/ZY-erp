...
- **查询**：分析单号（`MP_NO`）、订单号（`OS_NO`）下拉 + 查询 / 删除 / 保存
- **表格**：
  - 显示视图全部栏位
...
输出：`publish\TF_MP2视图维护.exe`（自包含，目标机无需安装 .NET）

或用 Visual Studio 2022 打开 `TfMp2Editor.sln` 发布。
...
字段映射定义在 `TfMp2Editor/AppConfig.cs`。

## 配置文件

登录勾选「记住连接信息」后，在 exe 同目录生成 `login_config.json`（密码明文，仅本机）。
